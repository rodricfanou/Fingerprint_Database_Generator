#!/usr/bin/env python
#./docker-compose.py up fanouroderick1 sxuyw68wEHXq9TEPxrnH
...
## It then finds out TLS fingerprints from provided samples, matches them with 
## HTTPS fingerprints and store the matches in a cross table
...
if __name__ == "__main__":
	...
	#print sys.argv, len(sys.argv)
		...
		username = sys.argv[2]
		key = sys.argv[3]
		postgresql_db_user = sys.argv[4]
		postgresql_db_pass = sys.argv[5]
		postgresql_db = sys.argv[6]
     	else:
		print "More parameters needed"
		print "Current Format: ./docker-compose.py up browserstack-user browserstack-key db-user db-pass db"
		sys.exit('STOP')


	### Create database by running the bash file edited for this purpose
	create_db =0
	if create_db == 1:
		print
        	print "1- Create database by running the bash file edited for this purpose"
		command = "bash create_tables_ubuntu.sh " + postgresql_db_user + " " + postgresql_db_pass  + " " + postgresql_db
		os.system(command)


	#sys.exit('OK for next step')
	## connexion to the posgresql  database
	## Fetch credentials for database
	print
	print "2- Fetch credentials for database"
	...
	if build_and_store:
		print
		print "3- Build HTTPS fingerprints"
		build_https_fgp(username, key, conn)


	## Fetching HTTP fingerprints
	print
	print "4- Fetch inserted HTTPS fingerprints"
	...
	print
	print "5- Fetch TLS fingerprints"
